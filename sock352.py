...
# The max packet size we will use. 60,000 bytes of data + 40 overhead
max_packet_size = 60040
...
class socket:
    ...
    def accept(self):
        ...
        while client_syn_packed_packet is None\
            ...
            (client_syn_packed_packet, client_addr) = self.udp_socket.recvfrom(max_packet_size)
    ...
    def recv(self, nbytes):
        ...
        # The data that we are returning
        buffer = b''
        # Remaining bytes that we expect to receive
        remainder = nbytes
        ...
        while remainder > 0:
            ...
            if packet.sequence_no == exp_seq_no:
                ...
                exp_seq_no += packet.payload_len
                # Add packet payload to our buffer
                buffer += packet.data

                # Send ACK for packet
                # We will use the sequence number field of this ACK packet to indicate how many bytes we received
                ack_packet = Packet(SOCK352_ACK, 0, exp_seq_no, 0, b'').pack_self()
                self.udp_socket.sendto(ack_packet, (self.sending_addr, send_port))

                # Subtract the amount received from remainder
                remainder -= packet.payload_len

        print('Server received the full transmission')
        return buffer
    ...
    def recv_packet(self):
        (struct_packet, recv_addr) = self.udp_socket.recvfrom(max_packet_size)
        # Unpacking header
